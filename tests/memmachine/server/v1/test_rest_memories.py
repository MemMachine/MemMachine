...
@pytest.mark.parametrize("missing_optional_session_field", [
    "group_id", "agent_id", "user_id"
])
def test_post_memories_missing_optional_nested_session_field(valid_post_payload, missing_optional_session_field):
    ...
    assert response.status_code == 200, f"Should succeed with missing optional session field: {missing_optional_session_field}"
...
def test_post_memories_boundary_empty_values(valid_post_payload):
    """Tests that POST requests fail with empty lists for optional session fields."""
    payload = valid_post_payload.copy()
    payload["session"]["group_id"] = "" # This is still valid
    payload["session"]["agent_id"] = [] # This will now fail with 422
    payload["session"]["user_id"] = [] # This will now fail with 422
    payload["producer"] = "" # This is still valid
    payload["episode_content"] = "" # This is still valid
    response = client.post("/v1/memories", json=payload)
    assert response.status_code == 422 # Expect 422 due to empty lists
...
def test_post_memories_none_optional_session_field(valid_post_payload, optional_field):
    ...
    response = client.post("/v1/memories", json=payload)
    assert response.status_code == 200, f"Should succeed with None optional session field: {optional_field}"
...
def test_post_memories_malicious_input(valid_post_payload, field, malicious_value):
    ...
    if field in ["agent_id", "user_id"]:
        ...
        valid_post_payload["session"][field] = malicious_value
    response = client.post("/v1/memories", json=valid_post_payload)
    assert response.status_code == 422, f"Should fail with malicious input in {field}: {malicious_value}"


@pytest.mark.parametrize("numerical_list, expected_status", [
    ...
    ([1.7976931348623157e+308, 1.0e+100], 422), # Large positive numbers (Python float max)
    ([-1.7976931348623157e+308, -1.0e+100], 422), # Large negative numbers
...
def test_post_memories_numerical_boundaries_episode_content(valid_post_payload, numerical_list, expected_status):
    """
    ...
    valid_post_payload["episode_type"] = "embedding"
    response = client.post("/v1/memories", content=json.dumps(valid_post_payload, allow_nan=True), headers={"Content-Type": "application/json"})
...
def test_search_valid_content_to_endpoints(valid_query_payload, endpoint, expected_keys):
...
def test_delete_memories_missing_optional_nested_session_field(valid_delete_payload, missing_optional_session_field):
...
@pytest.mark.parametrize("optional_field", ["group_id", "agent_id", "user_id"])
def test_delete_memories_none_optional_session_field(valid_delete_payload, optional_field):
