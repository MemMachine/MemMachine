...
class TestMemory:
    ...
    def test_delete_success(self, mock_client):
        ...
        call_args = mock_client.request.call_args
        assert call_args[0][0] == "POST"
        assert "/api/v2/memories/delete" in call_args[0][1]
        json_data = call_args[1]["json"]
        assert json_data["org_id"] == "test_org"
        assert json_data["project_id"] == "test_project"
        assert json_data["filter"] == ""

    def test_delete_with_timeout(self, mock_client):
        ...
        result = memory.delete(timeout=60)
    ...
    def test_delete_http_error(self, mock_client):
        """Test delete handles HTTP errors correctly."""
        mock_response = Mock()
        mock_response.status_code = 500
        mock_response.text = "Internal Server Error"
        mock_response.raise_for_status.side_effect = requests.HTTPError()
    ...
    def test_delete_client_closed(self, mock_client):
        """Test delete raises RuntimeError when client is closed."""
        memory = Memory(
            client=mock_client,
            org_id="test_org",
            project_id="test_project",
        ...
        with pytest.raises(
            RuntimeError, match="Cannot delete memories: client has been closed"
        ):
            memory.delete()
