...
<Note> There are two examples of `config.yml` files in the `sample_configs` directory of the GitHub repository: one for CPU-only installations and another for GPU-enabled setups. Be sure to choose the one that matches your environment.</Note>

**CPU-only installations:** You can download this file from our GitHub repository as a template using the following curl command:
```sh
curl -o config.yml https://raw.githubusercontent.com/MemMachine/MemMachine/refs/heads/main/sample_configs/episodic_memory_config.cpu.sample
```
...
**GPU-enabled installations:** You can download this file from our GitHub repository as a template using the following curl command:
```sh
curl -o config.yml https://raw.githubusercontent.com/MemMachine/MemMachine/refs/heads/main/sample_configs/episodic_memory_config.gpu.sample
```
