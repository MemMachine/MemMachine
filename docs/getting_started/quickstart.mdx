...
```
  ...
  <Tab title="PythonSDK" icon="python">
    ...
    <Note>Make sure to have your `.env` file and `config.yml` configuration file set up as described in the installation steps.</Note>
  ...
  async def main():
    await episodic_memory_test("config.yml")
