...
### For SearchMemory:
{
  ...
  "top_k": 10,
  "filter": "metadata.user_id='YourName'"
}

**Key differences from v1:**
- Use `org_id` and `project_id` (both default to "universal")
- Use `messages` array with `content` field (NOT `episode_content`)
- `user_id`, `agent_id`, `session_id`, `group_id` are stored in `metadata` as **strings** (NOT arrays)
- `producer` and `produced_for` are at the message level
- Use `top_k` instead of `limit` for search
- Use `filter` as a string: `"metadata.user_id='YourName'"`
...
3. **ALWAYS store new information** (AddMemory) if the user shared anything new:
   ...
   - **ALWAYS include metadata with user_id, agent_id, session_id, group_id**
