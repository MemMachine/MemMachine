...
The retrieval agent is created by
`src/memmachine/retrieval_agent/service_locator.py` and cached in
`MemMachine`. It is only used for long-term episodic retrieval.
