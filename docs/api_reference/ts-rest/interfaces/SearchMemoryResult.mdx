...
| `types` | [`MemoryType[]`](/api_reference/ts-rest/type-aliases/MemoryType) | *Optional.* Search within `episodic`, `semantic`, or both. |
...
* `short_term_memory`: Recent interactions as [`EpisodeMemory[]`](/api_reference/ts-rest/interfaces/EpisodeMemory).
* `long_term_memory`: Historical data as [`EpisodeMemory[]`](/api_reference/ts-rest/interfaces/EpisodeMemory).
* `episode_summary`: A list of strings summarizing the found episodes.

**Semantic Memory (`content.semantic_memory`)**
* Returns an array of [`SemanticMemory[]`](/api_reference/ts-rest/interfaces/SemanticMemory) representing distilled knowledge.
