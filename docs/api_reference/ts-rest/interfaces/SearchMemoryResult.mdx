...
| :--- | :--- | :--- |
| `top_k` | `number` | *Optional.* The maximum number of results to return. |
| `filter` | `string` | *Optional.* Filter expression string to narrow the search scope. |
| `types` | [`MemoryType[]`](../types/MemoryType) | *Optional.* Search within `episodic`, `semantic`, or both. |
| `agent_mode` | `boolean` | *Optional.* Enable retrieval-agent search mode. |
