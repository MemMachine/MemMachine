...
* `short_term_memory`: Recent interactions as [`EpisodeMemory[]`](./EpisodeMemory).
* `long_term_memory`: Historical data as [`EpisodeMemory[]`](./EpisodeMemory).
* `episode_summary`: A list of strings summarizing the found episodes.

**Semantic Memory (`content.semantic_memory`)**
* Returns an array of [`SemanticMemory[]`](./EpisodeMemory) representing distilled knowledge.
