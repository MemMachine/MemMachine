...
| `role` | [`MemoryProducerRole`](/api_reference/ts-rest/type-aliases/MemoryProducerRole) | *Optional.* The role of the creator (`user`, `assistant`, or `system`). |
...
| `types` | [`MemoryType[]`](/api_reference/ts-rest/type-aliases/MemoryType) | *Optional.* Specify if this should be stored as `episodic`, `semantic`, or both. |
