...
```typescript
await memory.search(query: string, options?: SearchMemoriesOptions)
```
...
| `options` | [`SearchMemoriesOptions`](../interfaces/SearchMemoryResult) | `top_k`, `filter`, `types`, and `agent_mode`. |

**Returns**

- `Promise<SearchMemoriesResult>` — Structured episodic and semantic retrieval output.
...
if (response.status === 0 && response.content.episodic_memory) {
  const recentMemories = response.content.episodic_memory.short_term_memory;
  if (recentMemories.length > 0) {
    console.log(`Found memory: ${recentMemories[0].content}`);
  }
}
```
