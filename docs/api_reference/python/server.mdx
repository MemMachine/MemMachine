...
- Manages both short-term and long-term episodic memory for a session key.
- Stores conversation-like events (episodes) and retrieves contextual history.
- Main system calls: `add_episodes()`, `query_search()`, `list_search()`.

### Episodic Memory Manager (`episodic_memory_manager`)

- Provides lifecycle and caching utilities for per-session episodic memories.
- Main manager calls: `open_episodic_memory()`,
  `open_or_create_episodic_memory()`, `delete_episodic_session()`.

### Semantic/Profile Memory (`semantic_memory`)

- Manages long-term semantic features (e.g., preferences, traits, facts).
- Main system calls: `add_features()`, `query_search()`, `list_search()`,
  `delete_features()`.

### Memory Types (`memory_types`)

- Defines the available target memories: `MemoryType.Episodic` and
  `MemoryType.Semantic`.
- Allows mixed operations through the same search/list APIs.
