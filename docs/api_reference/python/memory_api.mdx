...
```python
def search(query: str, limit: int | None, expand_context: int, score_threshold: float | None, agent_mode: bool, filter_dict: dict[str, str] | None, timeout: int | None)
```
...
| `agent_mode` | `bool` | Whether to use retrieval-agent search orchestration |
