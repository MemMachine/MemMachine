...
```python
def search(query: str, limit: int | None, expand_context: int, score_threshold: float | None, filter_dict: dict[str, str] | None, timeout: int | None, *, agent_mode: bool = False)
```
...
| `expand_context` | `int` | Number of neighboring episodic entries to include around each long-term match |
| `score_threshold` | `float | None` | Minimum episodic retrieval score to keep |
| `filter_dict` | `dict[str, str] | None` | Additional filters for the search |
| `timeout` | `int | None` | Request timeout in seconds (uses client default if not provided)  |
| `agent_mode` | `bool` | Whether to use retrieval-agent search orchestration (keyword-only; default `False`) |

**Returns**

`SearchResult` containing results from episodic and semantic memory
