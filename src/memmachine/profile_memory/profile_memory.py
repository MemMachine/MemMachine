...
class ProfileMemory:
    ...
    async def _background_ingestion_task(self):
        while not self._is_shutting_down:
            ...
            logger.info("DEBUG: ProfileMemory - Background task checking for dirty users: %s", dirty_users)

            if len(dirty_users) == 0:
                await asyncio.sleep(self.PROFILE_UPDATE_INTERVAL_SEC)
                continue

            logger.info("DEBUG: ProfileMemory - Processing uningested memories for users: %s", dirty_users)
    ...
    async def _process_uningested_memories(
        ...
        logger.info("DEBUG: ProfileMemory - Processing uningested memories for user: %s", user_id)
        message_isolation_groups = await self._get_isolation_grouped_memories(user_id)
        logger.info("DEBUG: ProfileMemory - Found %d message isolation groups for user %s", len(message_isolation_groups), user_id)

        async def process_messages(messages):
            if len(messages) == 0:
                logger.info("DEBUG: ProfileMemory - No messages to process")
                return

            logger.info("DEBUG: ProfileMemory - Processing %d messages", len(messages))
            mark_tasks = []

            for i in range(0, len(messages) - 1):
                message = messages[i]
                logger.info("DEBUG: ProfileMemory - Processing message %d for user %s", i, user_id)
                await self._update_user_profile_think(message)
                mark_tasks.append(
                    self._profile_storage.mark_messages_ingested([message["id"]])
                )

            logger.info("DEBUG: ProfileMemory - Processing last message with consolidation for user %s", user_id)
    ...
    async def _update_user_profile_think(
        ...
        logger.info("DEBUG: ProfileMemory - Calling LLM for profile update with user_id: %s", user_id)
        try:
            response_text, _ = await self._model.generate_response(
                system_prompt=self._update_prompt, user_prompt=user_prompt
            )
            logger.info("DEBUG: ProfileMemory - LLM response received for user_id: %s", user_id)
            logger.info("DEBUG: ProfileMemory - Raw LLM response for user_id %s: %s", user_id, response_text)
        ...
        logger.info("DEBUG: ProfileMemory - Extracted JSON for user %s: %s", user_id, response_json)

        # TODO: These really should not be raw data structures.
        try:
            profile_update_commands = json.loads(response_json)
            logger.info("DEBUG: ProfileMemory - Successfully parsed JSON for user %s: %s", user_id, profile_update_commands)
            ...
            try:
                ...
                if json_objects:
                    ...
                    logger.info("DEBUG: ProfileMemory - Successfully extracted JSON from malformed response for user %s: %s", user_id, profile_update_commands)
                else:
        ...
        logger.info("DEBUG: ProfileMemory - Executing %d valid commands for user %s", len(valid_commands), user_id)
        for command in valid_commands:
            if command["command"] == "add":
                logger.info("DEBUG: ProfileMemory - Adding profile feature for user %s: %s", user_id, command)
                ...
                logger.info("DEBUG: ProfileMemory - Successfully added profile feature for user %s", user_id)
            elif command["command"] == "delete":
                value = command["value"] if "value" in command else None
                logger.info("DEBUG: ProfileMemory - Deleting profile feature for user %s: %s", user_id, command)
    ...
    async def _deduplicate_profile(
        ...
        try:
            ...
            logger.info("DEBUG: ProfileMemory - Raw LLM response for consolidation: %s", response_text)
        except (ExternalServiceAPIError, ValueError, RuntimeError) as e:
        ...
        logger.info("DEBUG: ProfileMemory - Extracted JSON for consolidation: %s", response_json)
        try:
            updated_profile_entries = json.loads(response_json)
            logger.info("DEBUG: ProfileMemory - Successfully parsed JSON for consolidation: %s", updated_profile_entries)
        except ValueError as e:
            logger.warning(
                "Unable to load language model output '%s' as JSON, Error %s",
                str(response_json),
