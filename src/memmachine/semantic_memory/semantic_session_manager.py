...
from memmachine.common.errors import SetIdNotEnabledError
...
class SemanticSessionManager:
    ...
    async def add_feature(
        ...
        *,
        ...
        set_id = await self._get_single_set_id(
            session_data=session_data,
    ...
    async def _get_single_set_id(
        ...
        *,
        ...
        valid_set_id_tags = [set(sid.tags) for sid in org_set_ids]
        valid_set_id_tags.append(set())

        metadata_tags = set(metadata.keys())

        if metadata_tags not in valid_set_id_tags:
            raise SetIdNotEnabledError(
                org_id=session_data.org_id,
                project_id=session_data.project_id,
                is_org_level=is_org_level,
                metadata=metadata,
