...
def get_resources_status(resource_manager: ResourceManagerImpl) -> ResourcesStatus:
...
def _persist_config(resource_manager: ResourceManagerImpl) -> None:
    """Persist configuration changes to file if a config file path is set."""
    try:
        if resource_manager.config.config_file_path:
...
async def add_embedder(
    ...
    resource_manager.embedder_manager.add_embedder_config(
        name, provider, embedder_config
    )

    # Persist configuration to file
    _persist_config(resource_manager)

    # Attempt to build it immediately
    try:
        await resource_manager.embedder_manager.get_embedder(name)
    except Exception as e:
...
async def add_language_model(
    ...
    resource_manager.language_model_manager.add_language_model_config(
    ...
    # Attempt to build it immediately
    try:
        await resource_manager.language_model_manager.get_language_model(name)
    except Exception as e:
...
def remove_embedder(resource_manager: ResourceManagerImpl, name: str) -> bool:
    """Remove an embedder from the manager."""
    removed = resource_manager.embedder_manager.remove_embedder(name)
    if removed:
        _persist_config(resource_manager)
    return removed


def remove_language_model(resource_manager: ResourceManagerImpl, name: str) -> bool:
    """Remove a language model from the manager."""
    removed = resource_manager.language_model_manager.remove_language_model(name)
    if removed:
        _persist_config(resource_manager)
    return removed


async def retry_embedder(
    resource_manager: ResourceManagerImpl, name: str
) -> ResourceStatus:
    """
    ...
    try:
        await resource_manager.embedder_manager.retry_build(name)
...
async def retry_language_model(
    resource_manager: ResourceManagerImpl, name: str
) -> ResourceStatus:
    """
    ...
    try:
        await resource_manager.language_model_manager.retry_build(name)
...
async def retry_reranker(
    ...
    try:
        await resource_manager.reranker_manager.retry_build(name)
