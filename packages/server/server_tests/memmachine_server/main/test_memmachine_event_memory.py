...
def _make_resources(conf):
    ...
    mock_collection.config.indexed_properties_schema = {
        "_segment_uuid": str,
        "_timestamp": datetime,
    }
...
class TestEpisodeToEvent:
    def test_basic_conversion(self):
        ...
        assert event.context.context_type == "producer"
...
async def test_open_event_memory_disabled():
    ...
    resources = AsyncMock()
    # No tenant override either — _resolve_event_memory_conf falls back to None.
    mock_session_manager = AsyncMock()
    mock_session_manager.get_event_memory_conf = AsyncMock(return_value=None)
    resources.get_session_data_manager = AsyncMock(return_value=mock_session_manager)
...
async def test_open_event_memory_creates_new():
    ...
    mock_collection_new.config.indexed_properties_schema = {
        "_segment_uuid": str,
        "_timestamp": datetime,
    }
