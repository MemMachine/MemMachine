"""Strict loader for retrieval-agent specifications."""
...
def _validate_required_sections(*, body: str, required_sections: list[str]) -> None:
...
def _parse_raw_spec(raw_spec: dict[str, object] | str | Path) -> dict[str, object]:
    ...
    if not isinstance(parsed, dict):
        raise TypeError("Agent spec must deserialize to a JSON/YAML object.")

    return parsed


def load_agent_spec(raw_spec: dict[str, object] | str | Path) -> AgentSpecV1:
    """Load and validate a strict v1 retrieval-agent spec."""
    try:
        parsed_spec = _parse_raw_spec(raw_spec)
        return AgentSpecV1.model_validate(parsed_spec)
    except ValidationError as err:
        raise AgentContractError(
            code=AgentContractErrorCode.INVALID_SPEC,
            payload=AgentContractErrorPayload(
                what_failed="Agent specification validation failed",
                why=str(err),
                how_to_fix="Provide all required v1 fields and remove unknown keys.",
                where="agents.spec_loader.load_agent_spec",
                fallback_trigger_reason="invalid_agent_spec",
            ),
        ...
        raise AgentContractError(
            code=AgentContractErrorCode.INVALID_SPEC,
            payload=AgentContractErrorPayload(
                what_failed="Agent specification parsing failed",
                why=str(err),
